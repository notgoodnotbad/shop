...
<div class="form-div">
    ...
    <div class="list-div" id="listDiv">
        <input type="button" class="button ajax-post" value="删除" url="<?php echo U('changStatus');?>"/>
        <input type="button" class="button ajax-post" value="显示" url="<?php echo U('changStatus',array('status'=>1));?>"/>
        <input type="button" class="button ajax-post" value="隐藏" url="<?php echo U('changStatus',array('status'=>0));?>"/>
       
<table cellpadding="3" cellspacing="1">
    <tr>
        <th width="50px"><input class="selectAll" type="checkbox">ID</th>
        <th>供应商名称</th>
        <th>供应商描述</th>
        <th>排序</th>
        <th>是否显示</th>
    ...
    <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
            <td class="first-cell" width="50px"><input class="ids" type="checkbox" name="id[]" value="<?php echo ($row["id"]); ?>"><?php echo ($row["id"]); ?></td>
            <td class="first-cell"><?php echo ($row["name"]); ?></td>
            <td align="center"><?php echo ($row["intro"]); ?></td>
            <td align="center"><?php echo ($row["sort"]); ?></td>
            <td align="center"><a class="ajax-get" href="<?php echo U('changStatus',array('id'=>$row['id'],'status'=>(1-$row['status'])));?>"><img src="http://admin.shop.com/Public/Admin/images/<?php echo ($row["status"]); ?>.gif"/></a></td>
            <td align="center">
                <a href="<?php echo U('edit',array('id'=>$row['id']));?>" title="编辑">编辑</a> |
                <a  class="ajax-get" href="<?php echo U('changStatus',array('id'=>$row['id']));?>" title="移除">移除</a>
            </td>
